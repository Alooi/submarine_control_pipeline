...
bool sensor_init()
{
  uint8_t buf_whomi[1];
  uint8_t buf_stat[1];
  read(I2C_SLAVE_REG_WHO_AM_I, buf_whomi, 1);
  read(I2C_SLAVE_REG_STATUS, buf_stat, 1);

  if (buf_whomi[0] != MYMOTION_WHO_AM_I_VALUE)
  {
    Serial.println("WHO AM I FAILED");
    Serial.println(buf_whomi[0], HEX);
    return false;
  }
  if (buf_stat[0] != 0x80)
  {
    Serial.println("STATUS FAILED");
    Serial.println(buf_stat[0], HEX);
    return false;
  }
  return true;
}
...
int read_compensated_data()
{
  ...
  Serial.print("#COMP ACCEL = [");
  Serial.print(comp_acc_x);
  Serial.print(", ");
  Serial.print(comp_acc_y);
  Serial.print(", ");
  Serial.print(comp_acc_z);
  Serial.print("]  ");
  Serial.print("#COMP GYRO = [");
  Serial.print(comp_gyro_x);
  Serial.print(", ");
  Serial.print(comp_gyro_y);
  Serial.print(", ");
  Serial.print(comp_gyro_z);
  Serial.print("]  ");
  Serial.print("#COMP MAGNET = [");
  Serial.print(comp_mag_x);
  Serial.print(", ");
  Serial.print(comp_mag_y);
  Serial.print(", ");
  Serial.print(comp_mag_z);
  Serial.print("]  ");
  Serial.println();
}

// EULER ANGLE(RPY)
bool read_euler(float *euler)
{
  uint8_t buf_euler[6];
  // Check if read succeeds
  if (!read(I2C_SLAVE_REG_ROLL_LOW, buf_euler, 6))
  {
  ...
  // Convert to degrees
  float roll = (float)euler_x * 180.0 / 32767.0;
  float pitch = (float)euler_y * 180.0 / 32767.0;
  float yaw = (float)euler_z * 180.0 / 32767.0;
  ...
  memset(buf_euler, 0, sizeof(buf_euler));
